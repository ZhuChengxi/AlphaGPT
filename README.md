# AlphaGPT

> [!IMPORTANT]
> GitHub 的 issue 并非被设计为打卡工具！仅用于「打卡」「留名」的 issue 将会被直接删除。若您需要社群，请考虑 [DeepQuant](https://deepquant.one/) / QQ群组:1078080636。

> [!IMPORTANT]
> 若您在加密市场进行交易，另可参考 [Defense in Predatory Markets: A Differential Game Framework for AMM Liquidity via Uniswap V4 Hooks](https://github.com/imbue-bit/no_JIT) 进行做市。笔者懒得向会议投稿了。若有疑问，请联系 imbue2025@outlook.com. BTW，对该仓库代码进行 live trading 前作适当的修改可能会出现意想不到的业绩。

## What happened？

该项目是在一系列严重的劳动合规违规行为以及来自中国无锡一家量化交易公司的某成员的后续不断升级的投诉之后开源的。主要问题如下：

1. 非法雇佣未成年人：该个人在我 14 岁时明知故犯地与我建立了口头工作关系。经过数月的合作发展，他单方面终止了我们的合作，并在我的社交和职业圈内散布诽谤性的谣言。
2. 恶意的法律威胁与勒索：该个人试图通过捏造严重且毫无根据的刑事指控（如“信用卡欺诈”）来反悔之前的自愿性财务赠予（明确不是贷款）。他的明确意图是将民事问题刑事化，利用司法程序作为勒索的手段。
3. 利用脆弱性进行剥削：利用年龄、社会经验和权力动态方面的巨大差异，该个人进行了长期的心理压力施加。他威胁要使用不正当的行政手段来扰乱我们的运营，这是对未成年人合法权利的明显侵犯。非法持有及泄露个人数据威胁：他声称非法占有并威胁要泄露属于我和我家人的重要个人信息（SPI）。根据中国严格的个人信息保护法（PIPL），此类行为构成严重违规，可能会因侵犯公民个人信息而面临刑事指控。
4. 此次开源发布是对这些行为的直接回应。这是为了保护我们的权利，并对抗我们所遭遇的不道德行为所采取的必要措施。

![](hk.png)


## 去中心化

策略 IPFS CID：QmdZKy7rSAiRfRMpLvHGuVnWB8ENMDoGsQ6i3QGjqbzwMe

https://bafybeihcd2u6rsjtsq2rghuas3ln6jwbevfhktjbmugvgi4fb7p3osfsju.ipfs.dweb.link?filename=times.py

https://ipfs.io/ipns/k51qzi5uqu5djmgq90q9fhqmd2yxqhvqj8by1no448ulflqtygv2cv6j9vge3i

## 责任免除

在笔者受到威胁后，现在进一步开源我们在去中心化金融（DeFi）方面的工作。[使用金融数学方法在 Uniswap v4 中通过 Hooks 机制避免 JIT 攻击](./paper/20251226.pdf)

This repository is for research and educational purposes only. The author assumes no legal liability for any trading losses or systemic Alpha decay caused by the use of this code.

## Abstract

AlphaGPT 是一套基于深度学习算法与符号回归的量化因子挖掘框架。通过构建非线性、高阶的特征组合，在 A 股回测中表现出极强的鲁棒性与超额收益捕捉能力。

## Motivation

由于前雇主（某量化机构）在用工合规性上的严重缺失——包括但不限于非法雇佣未成年人、恶意低薪剥削——且双方并未建立任何法律意义上的保密义务。

我决定将此研究成果从开发仓库中使用 Cursor 提取核心逻辑并开源。我不在乎这些代码能赚多少钱，我只在乎让他们无钱可赚。

欢迎社区共同参与，加速该策略在 A 股市场的熵增过程。

## This was their money-making machine. Now it's your public library.

中证1000

![](backtest.png)

国债十年

![](backtest_2.png)

该逻辑同样在加密市场适用

![](crypto_2.png)

## OH! NO!

Upd 2026.01.08: 笔者的 GitHub 仓库可能无法更新了!

![](funny_1.png)
![](funny_2.png)
![](funny_3.png)
![](funny_4.png)

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=imbue-bit/AlphaGPT&type=date&legend=top-left)](https://www.star-history.com/#imbue-bit/AlphaGPT&type=date&legend=top-left)
